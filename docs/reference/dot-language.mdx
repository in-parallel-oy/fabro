...
| `agent` | String | Agent implementation to use. `api` (default): Fabro manages the tool loop. `cli`: Fabro delegates to an external assistant CLI (`claude`, `codex`, or `gemini`). Any other value must match a named agent in the `[agents]` configuration (see [ACP Integration](/agents/acp)). |
