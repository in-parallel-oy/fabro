...
The button launches Railway's template flow, which deploys the pre-built `ghcr.io/in-parallel-oy/fabro:nightly` image directly from GHCR — no Rust compilation on Railway's builder, so deploys complete in seconds.
...
Railway re-pulls the GHCR image on every deploy. The template uses the `:nightly` tag by default, so redeploying picks up the latest nightly automatically. To pin a specific version, change the image in **Service → Settings → Source** to `ghcr.io/in-parallel-oy/fabro:<version>` and redeploy. The `/storage` Volume survives redeploys, so runs and checkpoints persist.
