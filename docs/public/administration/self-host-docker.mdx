...
The supported deployment artifact is the official Fabro image at `ghcr.io/in-parallel-oy/fabro`. Everything else — `docker compose`, ECS, Cloud Run, Kubernetes, Railway — is just running this image somewhere with the right requirements.

## Requirements

| Requirement | Value |
|---|---|
| **Image** | `ghcr.io/in-parallel-oy/fabro:nightly` (multi-arch; pin a version for production) |
...
```bash
git clone https://github.com/in-parallel-oy/fabro.git
cd fabro
cp .env.example .env
...
- Pulls `ghcr.io/in-parallel-oy/fabro:nightly`
...
See [Server Configuration](/administration/server-configuration) for the full settings reference, and [`.env.example`](https://github.com/in-parallel-oy/fabro/blob/main/.env.example) for the complete list.

## Cloud container services

The same image works on any container orchestrator that supports the requirements above. Common patterns:

- **AWS ECS / Fargate** — Task definition referencing `ghcr.io/in-parallel-oy/fabro:nightly`, EFS volume mounted at `/storage`, port `32276` published, environment variables for bootstrap values, and vault-backed optional integration secrets.
...
`docker-compose.yaml` uses `:nightly` by default, so `docker compose pull && docker compose up -d` picks up the latest nightly. To pin a specific version, change the `image:` line to `ghcr.io/in-parallel-oy/fabro:<version>`.
