...
<Note>
The tools described on this page apply to the **API agent** (the default). When using the [CLI agent](/core-concepts/agents#cli-agent), the external CLI tool (`claude`, `codex`, or `gemini`) provides its own tools — Fabro's built-in tools are not used.
</Note>
