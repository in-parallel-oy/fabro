...
Every agent node uses an **agent implementation** that determines how Fabro interacts with the LLM. There are three primary types:
...
Fabro delegates execution to an external agent that implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/). The ACP agent runs inside the sandbox and communicates with Fabro over stdio. Fabro intercepts tool permission requests to enforce security policies and HITL guards.

You can configure multiple custom agents in your `run.toml` and select them by name.

See [ACP Agent Integration](/agents/acp) for details.
...
Set the CLI agent on a node with `agent="cli"` or via a [model stylesheet](/workflows/stylesheets):

```dot
implement [label="Implement", agent="cli"]
```
...
| Capability | API agent | Custom agent (ACP) | CLI agent |
|---|---|---|---|
| Tools | Fabro built-in tools + MCP | Agent's own tools | CLI's own tool set |
| Session caching | Supported (`fidelity` + `thread_id`) | Not supported | Not supported |
| Sub-agents | Supported | Not supported | Not supported |
| Provider failover | Supported | Not supported | Not supported |
| File tracking | Tool call events | Tool call events | `git diff` before/after |
| Security hooks | Supported | Supported | Not supported |

### When to use custom ACP agents

- **ACP-compatible agents** — leverage external agents that implement the Agent Client Protocol (e.g., `claude-agent-acp`, `opencode acp`)
- **Advanced agent capabilities** — use agents with specific capabilities like LSP integration, custom skills, or rules that are not natively available in Fabro
- **Sandboxed execution** — run external agents securely inside Fabro's execution environments with HITL guards

### When to use the CLI agent
