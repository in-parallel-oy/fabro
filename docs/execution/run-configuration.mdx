...
Configure external agents to run inside the sandbox instead of the built-in Fabro agent. Each entry in the map defines a named agent configuration.
...
| Field | Description |
|---|---|
| `type` | Agent protocol type (e.g., `"acp"` for Agent Client Protocol). |
| `command` | The shell command to start the agent inside the sandbox. |

Select the agent on a node using the `agent` attribute in DOT, or set a default via the `$agent` variable in `[vars]`.

See [ACP Agent Integration](/agents/acp) for more details.
...
The `fabro.toml` project config can set default values for `[llm]`, `[agents]`, `[setup]`, `[sandbox]`, `[vars]`, `[checkpoint]`, `[pull_request]`, `[github]`, `[assets]`, `[[hooks]]`, and `[mcp_servers]`. These defaults apply to all runs in the project unless the run config overrides them:
