...
Configure an Agent Client Protocol (ACP) agent to run inside the sandbox instead of the built-in Fabro agent.
...
The `fabro.toml` project config can set default values for `[llm]`, `[acp]`, `[setup]`, `[sandbox]`, `[vars]`, `[checkpoint]`, `[pull_request]`, `[github]`, `[assets]`, `[[hooks]]`, and `[mcp_servers]`. These defaults apply to all runs in the project unless the run config overrides them:
