...
| `agent` | Agent implementation — `api` (default) runs Fabro's own tool loop, `cli` delegates to an external CLI tool. See [Agent Implementations](/core-concepts/agents#agent-implementations). | `cli`, `api` |
